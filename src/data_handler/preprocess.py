...
def whitespace_tokenization(text_column, lowercase=False):
    if lowercase:
...
def tokenize_column(text_column, tokenization_type="whitespace", n_tokens=1, stopwords=None, lowercase=False):
    """"""
    #TODO take an array/series of texts and tokenize it, return same array/series but tokenized
    if tokenization_type == "whitespace":
        if n_tokens == 1:
            tokenized_text_column = whitespace_tokenization(text_column, lowercase=False)
        else:
...
def tokenize_add_tok_column(input_dataframe, col_names_dict, n_tokens=1, stopwords=None, lowercase=False):
    ...
    for text_column in col_names_dict[utils.TEXT_COLS_KEY]:
        ...
        input_dataframe[tokenized_col_names[text_column]] = tokenize_column(
            input_dataframe[[str(text_column)]], n_tokens=n_tokens, stopwords=None, lowercase=False)
