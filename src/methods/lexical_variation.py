...
def maas(label_values_dict, subsets_of_interest, args):  
    ...
    for label in freqs_dict:
        maas_dict[label] = safe_divide(
            (math.log10(tokens_dict[label]) - math.log10(types_dict[label])), 
            math.pow(math.log10(tokens_dict[label]), 2)
        )
...
def lttr(label_values_dict, subsets_of_interest, args):  
    ...
    for label in freqs_dict:
        lttr_dict[label] = safe_divide(
            math.log10(types_dict[label]), 
            math.log10(tokens_dict[label])
        )
