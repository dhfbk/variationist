...
class BinnedGeoChart(AltairChart):
    """A class for building a BinnedGeoChart object."""
    ...
    def __init__(
        ...
        """
        Initialization function for a building a BinnedGeoChart object.
        ...
        # Get the shapefile min/max for each dimension
        # gdf_min_x, gdf_min_y, gdf_max_x, gdf_max_y = gdf.total_bounds

        # Calculate the size of the bins (width, height)
        chart_base_size = 600
        num_lat_bins, num_lon_bins = metadata["var_bins"][0], metadata["var_bins"][1] # @TODO: Generalize to 4+ dims
        bin_width = chart_base_size / num_lon_bins * 0.75 # @TODO: Workaround, to test to many scenarios
        bin_height = chart_base_size / num_lat_bins * 0.95 # @TODO: Workaround, to test to many scenarios
        ...
        self.base_chart = self.base_chart.mark_rect(
            width=bin_width, height=bin_height, strokeWidth=0.5)
        ...
        # Convert the bounding boxes to centroids
        lat_centroids = []
        lon_centroids = []
        for index, row in df_data.iterrows():
            lat_min, lat_max = row[lat_name][1:-1].split(", ")
            lon_min, lon_max = row[lon_name][1:-1].split(", ")
            lat_centroids.append((float(lat_min) + float(lat_max)) / 2)
            lon_centroids.append((float(lon_min) + float(lon_max)) / 2)
        df_data[lat_name] = lat_centroids
        df_data[lon_name] = lon_centroids
